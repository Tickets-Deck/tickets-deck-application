...
const ProfilePage: FunctionComponent<ProfilePageProps> = (): ReactElement => {
    ...
    async function handleFetchUserBankAccount() {
        ...
        await fetchUserBankAccount(user?.token as string, userInformation?.id as string)
            .then((response) => {
        ...
        <div className=''>
            {userInformation && (
                <>
                            ...
                            <div className='w-full flex flex-col'>
                                ...
                                <div className='flex items-center justify-between py-2 px-4 border-y border-dark-grey/10 cursor-pointer hover:bg-dark-grey/[0.035]'>
                                    <p className='text-primary-color text-xs'>
                                        <Link
                                            target='_blank'
                                            href={`${window.location.origin}/u/${userInformation.username || userInformation.id
                                                }`}
                                        >
                            ...
                            <div className='w-full flex items-center max-[768px]:overflow-x-auto'>
                                {
                                    Object.values(ProfilePageTab).map((tab) => {
                                        if (isNaN(Number(tab)) || tab == ProfilePageTab.IdentityVerification) {
                                            return;
                                        }
                                        return (
                                            <span
                    ...
                    ${currentTab === tab ? "border-primary-color bg-dark-grey/5 opacity-100 pointer-events-none" : "opacity-50 border-transparent"}`
                                                }
