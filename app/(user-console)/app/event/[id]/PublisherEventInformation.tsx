...
import { useDeleteEvent, useDeleteTicket, useFetchEventTickets, useFetchEventViewsCount, useFetchPublisherEventById, useUpdateEventById } from "@/app/api/apiClient";
...
const PublisherEventInformation: FunctionComponent<PublisherEventInformationProps> = ({ id, session }): ReactElement => {
                    ...
                    <div className="flex flex-row justify-between items-center gap-4 mb-10">
                        <div>
                            ...
                            <div className="flex flex-row items-center gap-2">
                                <p className="text-sm text-white/60">Posted on: {moment(eventInfo.createdAt).format("ddd, MMM Do, YYYY, hh:mm A")}</p>
                                {
                                    eventViewsCount ?
                                        <>
                                            <span>|</span>
                                            <p className="text-sm text-white/60 flex flex-row items-center gap-1"><Icons.Eye width={16} height={16} /> {eventViewsCount}</p>
                                        </> : <></>
                                }
                            </div>
                        </div>
