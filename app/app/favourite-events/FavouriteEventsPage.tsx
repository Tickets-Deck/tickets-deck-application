...
const FavouriteEventsPage: FunctionComponent<FavouriteEventsPageProps> = (): ReactElement => {

    const fetchUserFavoriteEvents = useFetchUserFavoriteEvents();
    const { data: session, status } = useSession();
    const user = session?.user;

    const [isFetchingFavouriteEvents, setIsFetchingFavouriteEvents] = useState(true);
    const [favouriteEvents, setFavouriteEvents] = useState<EventResponse[] | undefined>(undefined);
    ...
    return (
        <main className={`${sectionPadding} flex-col gap-8 py-16 bg-dark-grey flex md:flow-row items-start relative min-h-[calc(100vh_-_56px)]`}>
            ...
            <div className="flex flex-col w-full">
                ...
                {
                    !isFetchingFavouriteEvents && (favouriteEvents && favouriteEvents?.length == 0) &&
                ...
                }
                {
                    ...
                    <div className="grid place-items-center">
                        ...
                        <p className="text-white/80 text-center">
                            An error occurred while fetching your favourite events.<br />Please try again later.
                        </p>
                    </div>
                }
            </div>
