"use client"
import { useFetchUserPayouts, useFetchUserWalletBalance, useInitiateWitdrawal } from "@/app/api/apiClient";
...
const WalletPage: FunctionComponent<WalletPageProps> = (): ReactElement => {
    ...
    const userBankAccount = useSelector((state: RootState) => state.wallet.bankAccount);
            ...
            <main className="px-4 py-8 min-h-screen">
                ...
                <div className="w-full grid grid-cols-2 gap-4">
                    <div className="p-6 rounded-2xl flex flex-col bg-dark-grey">
                        <span className="text-2xl font-semibold mb-4 font-Mona-Sans-Wide">&#8358;{walletBalance && showWalletBalance ? (walletBalance).toLocaleString() : "****"}</span>
                            ...
                            <table className="bg-white w-full text-dark-grey">
                                <tbody>
                                    <tr>
                                        ...
                                        <th className="p-3 border-b-[1px] text-left text-sm font-semibold">Date Requested</th>
                                        <th className="p-3 border-b-[1px] text-left text-sm font-semibold">Date Paid</th>
...

                                    {
                                        userPayouts.map((payout, index) =>
                                            <tr key={index}>
                                                <td className="p-3 border-b-[1px] text-left text-sm">{payout.transactionRef ?? "Unavailable"}</td>
                                                <td className="p-3 border-b-[1px] text-left text-sm">&#8358;{payout.amount}</td>
                                                <td className="p-3 border-b-[1px] text-left text-sm">{moment(payout.createdAt).format("Do MMM, YYYY | hh:mma")}</td>
                                                <td className="p-3 border-b-[1px] text-left text-sm">{payout.payoutDate ? moment(payout.payoutDate).format("Do MMM, YYYY | hh:mma") : "Unavailable"}</td>
                                                <td className="p-3 border-b-[1px] text-left text-sm">{payout.status}</td>
                                                <td className="p-3 border-b-[1px] text-left text-sm">{payout.paymentMethod}</td>
                                                <td className="p-3 border-b-[1px] text-left text-sm">{`${payout.serviceFees ?? 0}%`}</td>
                                                <td className="p-3 border-b-[1px] text-left text-sm">{`${payout.tax ?? 0}%`}</td>
