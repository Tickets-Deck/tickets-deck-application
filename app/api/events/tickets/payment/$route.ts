...
export async function GET(req: NextRequest) {
  if (req.method === "GET") {
    ...
    if (paymentResult.status === true) {
      ...
      // Using the ticketId for each ordered ticket, update the ticket ordered count of each ticket
      const orderedTickets = await prisma.orderedTickets.findMany({
        where: {
          orderId: updatedTicketOrders.id,
        },
      });

      // Update the ticket ordered count of each ticket
      for (const orderedTicket of orderedTickets) {
        await prisma.tickets.update({
          where: {
            id: orderedTicket.ticketId,
          },
          data: {
            ticketOrdersCount: {
              increment: 1,
      ...
      const eventPublisher = await prisma.users.findUnique({
        where: {
          id: updatedEvent.publisherId,
        },
      });

      // If the event publisher is found, update the ticketSold count of the event publisher
      if (eventPublisher) {
        ...
        await prisma.users.update({
          ...
          data: {
            ticketsSold: eventPublisher.ticketsSold + 1,
          },
        });
      }
      ...
      if (updatedTicketOrders.userId) {
        // Find the user who bought the ticket
        const ticketBuyer = await prisma.users.findFirst({
          where: {
            id: updatedTicketOrders.userId,
          },
        });

        // Update the tickets bought count for the user
        if (ticketBuyer) {
          await prisma.users.update({
            where: {
              id: updatedTicketOrders.userId,
            ...
            data: {
              ticketsBought: ticketBuyer.ticketsBought + 1,
            },
          });
        }
      }
