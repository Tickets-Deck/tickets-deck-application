...
export function CheckInArea({ eventId, isLive }: CheckInAreaProps) {
            ...
            .catch((error) => {
                ...
                if (error?.response) {
                    ...
                    if (error.response.data.errorCode == ApplicationError.TicketOrderWithIdNotFound.Code) {
                        toasthandler?.logError("Error checking in", "Invalid event access code");
                        return;
                    }
                    if (error.response.data.errorCode == ApplicationError.TicketOrderHasBeenCheckedIn.Code) {
                        toasthandler?.logError("Error checking in", "This ticket order has been checked in.");
                        return;
                    }
                    toasthandler?.logError("Error checking in", "An error occurred while checking in. Please try again.");
                }
            })
            .finally(() => {
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div>
                        <div className="flex flex-col md:flex-row gap-4 mb-6">
                            ...
                            <div className="ml-auto md:m-0 flex flex-row items-center gap-2">
                                <Button
                                    minBtn
                                    className={`!bg-white !text-black hover:!opacity-50 flex flex-row items-center !gap-1 w-fit text-nowrap ${!typedAccessCode ? 'opacity-50 pointer-events-none' : ''}`}
                    ...
                    <div className="space-y-4">
                        {scannedAccessCode &&
                            mockAttendees.map((attendee) => (
                                <div key={attendee.id} className="flex items-center justify-between p-4 rounded-lg bg-[#252525]">
                                    <div>
          ...
          ${checkInSuccess === null ? "translate-y-full opacity-0" : "translate-y-0 opacity-100"}
        `}
                >
                    <div className="flex items-center gap-2">
                        {checkInSuccess ? (
